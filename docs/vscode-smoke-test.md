...
GitHub and CI workflow context also stays local-first in the no-paid smoke path. Unit tests cover `Veyra: Summarize Git Status`, which uses read-only Git commands to summarize branch/upstream state, sanitized remotes, dirty-tree files, and the latest commit. The flow provides GitHub PR and CI follow-up guidance with no hidden network automation and no automatic pushes; it does not run push, pull, merge, rebase, reset, clean, GitHub CLI, or CI commands.
