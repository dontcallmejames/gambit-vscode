...
- **Unhandled stdin EPIPE can crash the host** (R1). *Rec:* `child.stdin?.on('error', ...)` in all three agents + a top-level `process.on('uncaughtException'|'unhandledRejection')` guard logging to the output channel. **(High, S)**
