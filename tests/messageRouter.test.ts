...
describe('MessageRouter', () => {
  ...
  it('gives concrete Gemini setup guidance when the direct backend is not installed', async () => {
    ...
    expect(sendSpy).not.toHaveBeenCalled();
    expect(events).toEqual([
      { kind: 'routing-needed', text: 'Google provider CLI is not installed. Install Antigravity CLI from https://antigravity.google/cli, then run `agy` once if sign-in is needed. Legacy Gemini CLI remains a fallback for existing/API-key users. You can also run Veyra: Configure Codex/Gemini CLI paths or Veyra: Show setup guide.' },
    ]);
  });
  ...
  it('gives CLI override guidance when a direct Gemini backend is misconfigured', async () => {
    ...
    expect(sendSpy).not.toHaveBeenCalled();
    expect(events).toEqual([
      { kind: 'routing-needed', text: 'Google provider CLI path is misconfigured. Set VEYRA_ANTIGRAVITY_CLI_PATH / veyra.antigravityCliPath to agy.exe, or set VEYRA_GEMINI_CLI_PATH / veyra.geminiCliPath to gemini.js, gemini.exe, or gemini. You can also run Veyra: Configure Codex/Gemini CLI paths or Veyra: Show setup guide.' },
    ]);
  });
